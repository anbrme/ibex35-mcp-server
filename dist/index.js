            ...
            try {
                ...
                switch (name) {
                    ...
                    case 'get_company_by_symbol':
                        result = await this.db.getCompanyBySymbol(args?.symbol);
                        break;
                    case 'get_companies_by_sector':
                        result = await this.db.getCompaniesBySector(args?.sector);
                        break;
                    case 'get_companies_with_pe_ratio':
                        result = await this.db.getCompaniesWithPERatio(args?.minPE, args?.maxPE);
                        break;
                    ...
                    case 'get_directors_by_name':
                        result = await this.db.getDirectorsByName(args?.name);
                        break;
                    case 'get_company_shareholders':
                        result = await this.db.getCompanyShareholders(args?.companyId);
                        break;
                    case 'get_shareholder_overlap':
                        result = await this.db.getShareholderOverlap();
                        break;
                    case 'get_top_shareholders_by_sector':
                        result = await this.db.getTopShareholdersBySector(args?.sector, args?.limit || 10);
                        break;
                    case 'get_historical_prices':
                        result = await this.db.getHistoricalPrices(args?.companyId, args?.days || 30);
                        break;
                    case 'get_top_performers':
                        result = await this.db.getTopPerformers(args?.days || 7, args?.limit || 10);
                        break;
                    case 'get_recent_news':
                        result = await this.db.getRecentNews(args?.companyId, args?.limit || 20);
                        break;
                    case 'get_news_by_sentiment':
                        result = await this.db.getNewsBySentiment(args?.sentiment, args?.limit || 20);
                        break;
                    case 'get_lobbying_meetings':
                        result = await this.db.getLobbyingMeetings(args?.companyId, args?.limit || 20);
                        break;
                    case 'get_most_active_lobbyists':
                        result = await this.db.getMostActiveLobbyists(args?.limit || 10);
                        break;
                    case 'get_esg_scores':
                        result = await this.db.getESGScores(args?.companyId);
                        break;
                    case 'get_weekly_reports':
                        result = await this.db.getWeeklyReports(args?.reportType, args?.limit || 10);
                        break;
                    ...
                    case 'analyze_natural_query':
                        result = await this.analytics.analyzeNaturalQuery(args?.query, args?.context);
                        break;
                    case 'compare_companies':
                        result = await this.analytics.compareCompanies(args?.companies, args?.metrics || ['all']);
                        break;
                    case 'analyze_trends':
                        result = await this.analytics.analyzeTrends(args?.analysis_type, args?.target, args?.period || 30, args?.include_forecast || false);
                        break;
                    case 'assess_investment_risk':
                        result = await this.analytics.assessInvestmentRisk(args?.target, args?.risk_types || ['all']);
                        break;
                    case 'generate_analyst_report':
                        result = await this.analytics.generateAnalystReport(args?.subject, args?.report_type, args?.include_charts !== false);
                        break;
                    case 'screen_opportunities':
                        result = await this.analytics.screenOpportunities(args?.screening_criteria, args?.limit || 10);
                        break;
