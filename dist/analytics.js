export class AnalyticsManager {
    ...
    async analyzeNaturalQuery(query, context) {
            ...
            // Comparison queries
                ...
                for (const pattern of companyPatterns) {
                    if (lowerQuery.match(pattern)) {
                        ...
                        if (match)
                            foundCompanies.push(match[0].toUpperCase());
                    }
                }
    ...
    async compareCompanies(companies, metrics = ['all']) {
        try {
            const companyData = [];
    ...
    async analyzeTrends(analysisType, target, period = 30, includeForecast = false) {
        try {
            const analysis = {
            ...
            switch (analysisType) {
                case 'company_trend':
                    ...
                    if (!company)
                        throw new Error(`Company ${target} not found`);
    ...
    async assessInvestmentRisk(target, riskTypes = ['all']) {
        try {
    ...
    async generateAnalystReport(subject, reportType, includeCharts = true) {
        try {
            const report = {
                subject: subject,
            ...
            switch (reportType) {
                case 'company_deep_dive':
                    ...
                    if (!company)
                        throw new Error(`Company ${subject} not found`);
                    report.sections = {
    ...
    async screenOpportunities(criteria, limit = 10) {
        try {
            const companies = await this.db.getAllCompanies();
            const screening = {
                criteria: criteria,
            ...
            if (criteria.pe_ratio_min !== undefined || criteria.pe_ratio_max !== undefined) {
                filteredCompanies = filteredCompanies.filter(company => {
                    ...
                    if (pe === null || pe === undefined)
                        return false;
                    if (criteria.pe_ratio_min !== undefined && pe < criteria.pe_ratio_min)
                        return false;
                    if (criteria.pe_ratio_max !== undefined && pe > criteria.pe_ratio_max)
                        return false;
                    return true;
                });
            }
            if (criteria.market_cap_min !== undefined || criteria.market_cap_max !== undefined) {
                filteredCompanies = filteredCompanies.filter(company => {
                    if (!company.market_cap)
                        return false;
                    if (criteria.market_cap_min !== undefined && company.market_cap < criteria.market_cap_min)
                        return false;
                    if (criteria.market_cap_max !== undefined && company.market_cap > criteria.market_cap_max)
                        return false;
                    return true;
                });
            }
            if (criteria.sectors && criteria.sectors.length > 0) {
                filteredCompanies = filteredCompanies.filter(company => criteria.sectors.some(sector => company.sector?.toLowerCase().includes(sector.toLowerCase())));
            }
            if (criteria.exclude_sectors && criteria.exclude_sectors.length > 0) {
                filteredCompanies = filteredCompanies.filter(company => !criteria.exclude_sectors.some(sector => company.sector?.toLowerCase().includes(sector.toLowerCase())));
            }
    ...
    generateTrendSummary(topPerformers) {
    ...
    generateActionSuggestions(interpretation, results) {
        const suggestions = [];
    ...
    async analyzeCompanyPerformances(companies, period) {
        const performances = [];
    ...
    calculatePerformance(historical) {
        if (historical.length < 2)
            return null;
        const recent = historical[0];
        const old = historical[historical.length - 1];
    ...
    assessCompanyRisk(company, directors) {
        let riskLevel = 'low';
        if (directors.length > 15)
            riskLevel = 'medium'; // Large board
        if (!company.market_cap || company.market_cap < 1000000000)
            riskLevel = 'medium'; // Small cap
        if (!company.price_to_earnings && !company.pe_ratio)
            riskLevel = 'high'; // No PE data
        return riskLevel;
    }
    generateComparisonSummary(companyData) {
        const validCompanies = companyData.filter(c => !c.error);
        if (validCompanies.length === 0)
            return { error: 'No valid companies to compare' };
        return {
            companies_analyzed: validCompanies.length,
            best_performer: validCompanies.reduce((best, current) => (current.recent_performance?.period_change || -Infinity) >
                (best.recent_performance?.period_change || -Infinity) ? current : best),
            most_covered: validCompanies.reduce((most, current) => (current.news_coverage || 0) > (most.news_coverage || 0) ? current : most),
    ...
    generateComparisonRecommendations(companyData) {
        ...
        const bestPerformer = validCompanies.reduce((best, current) => (current.recent_performance?.period_change || -Infinity) >
            (best.recent_performance?.period_change || -Infinity) ? current : best);
    ...
    analyzePriceTrend(historical) {
        ...
        if (position > 0.7)
            trend.current_position = 'near_resistance';
        else if (position < 0.3)
            trend.current_position = 'near_support';
        return trend;
    }
    generateTrendInsights(trendData, company) {
        const insights = [];
        if (!trendData) {
    ...
    generateSimpleForecast(historical) {
    ...
    calculateMarketVolatility(performances) {
    ...
    async assessMarketRisk(targetData, targetType) {
        return {
            risk_level: 'medium',
    ...
    async assessGovernanceRisk(targetData, targetType) {
        return {
            risk_level: 'low',
    ...
    async assessSectorRisk(targetData, targetType) {
        return {
            risk_level: 'medium',
    ...
    async assessLiquidityRisk(targetData, targetType) {
        return {
            risk_level: 'low',
    ...
    async assessConcentrationRisk(targetData, targetType) {
        return {
            risk_level: 'medium',
    ...
    calculateOverallRiskScore(riskBreakdown) {
        const scores = Object.values(riskBreakdown).map((risk) => risk.score || 5);
        return scores.reduce((sum, score) => sum + score, 0) / scores.length;
    }
    generateRiskRecommendations(riskBreakdown) {
        const recommendations = [];
        Object.entries(riskBreakdown).forEach(([riskType, riskData]) => {
    ...
    async getCompanyGovernanceAnalysis(company) {
        const directors = await this.db.getCompanyDirectors(company.id);
        const interlocks = await this.db.getBoardInterlocks();
        const companyInterlocks = interlocks.filter(interlock => interlock.companies.includes(company.name));
        return {
    ...
    async getCompanyMarketPosition(company) {
    ...
    generateCompanyExecutiveSummary(sections) {
    ...
    async analyzeSectorCompetition(companies) {
        return {
            total_companies: companies.length,
    ...
    generateReportConclusions(report) {
        return [
    ...
    async scoreOpportunities(companies, criteria) {
        const scored = companies.map(company => {
            ...
            if (company.market_cap > 10e9)
                score += 0.5; // Large cap bonus
            if (company.market_cap < 1e9)
                score -= 0.3; // Small cap penalty
    ...
    getTopSector(opportunities) {
        ...
        return Object.entries(sectorCounts).reduce((top, [sector, count]) => count > (sectorCounts[top] || 0) ? sector : top, 'Other');
    }
    calculateGovernanceScore(directors, interlocks) {
        let score = 8.0; // Base good governance score
        if (directors.length > 15)
            score -= 1.0; // Too large board
        if (directors.length < 5)
            score -= 0.5; // Too small board
        if (interlocks.length > 5)
            score -= 2.0; // Too many interlocks
    ...
    calculateConcentrationRatio(companies) {
